// Package seed — observations.go: the user-curated ~/.ghost/seed-observations.json
// file. Each entry is a high-confidence observation authored directly by the user
// (via `ghost remember`). extract never writes this file; the cluster stage loads
// it alongside the extracted observations so the fact routes through the normal
// generality + confidence gates and survives recompose.
package seed
...
// LoadSeedObservations reads the seed-observations file. A missing file is not an
// error (absent file = no user observations): it returns a zero ObservationsFile.
...
// AppendSeedObservation validates o, then appends it to the file at path,
// creating the file (with the fixed seed envelope) if absent. Atomic write.
...
// re-runs clustering/synthesis. Confidence and evidence are excluded
// intentionally — changes to those fields alone do not re-trigger reclustering,
// which is safe because `ghost remember` always writes fixed high confidence
// and generated evidence; the meaningful content is kind+text.
